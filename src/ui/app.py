...
# Page Config
# 1. 保持 layout="wide"，确保页面框架（标题、Tab栏）始终撑满屏幕，不会因切换内容而跳动
...
def main():
    ...
    st.markdown("""
        <style>
        /* 1. 顶部留白调整 (不再限制全局 max-width，解决割裂感) */
        .block-container {
        ...
        /* 使用 :has() 选择器匹配 aria-label 属性，精准命中特定名字的输入框 */
        ...
        /* 针对上面选中的输入框：让输入框内的数字 (Input Text) 也居中 (可选，如果你想让数字也居中) */
    ...
    # --- Tab 1: 单品计算器 (核心修改：内部居中布局) ---
    with tab1:
        # 【关键代码】使用列布局 [1, 2, 1] 将内容“挤”在中间
        # 这使得 Tab1 看起来紧凑，而 Tab2 依然可以全宽，且 Tab 栏位置不动
    ...
    # --- Tab 2: 批量计算器 (Tab 2 保持默认，自动使用 Wide 全屏) ---
    with tab2:
        st.header("📂 批量费用计算")
        st.markdown(
